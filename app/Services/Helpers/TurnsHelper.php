...
class TurnsHelper {
    ...
    public static function JoinEventByDate($microsite_id, $date) {
        ...
        $lastTurn = \App\Entities\ev_event::select(array(
                            "res_turn.*",
                            DB::raw("CONCAT('" . $date . "',' ',hours_ini) AS start_datetime"),
                            DB::raw("'" . $date . "' AS start_date"),
                            DB::raw("IF(hours_end > hours_ini, '" . $date . "', '" . $nextday->toDateString() . "') AS end_date"),
                            DB::raw("IF(hours_end > hours_ini, CONCAT('" . $date . "',' ',hours_end), CONCAT('" . $nextday->toDateString() . "',' ',hours_end)) AS end_datetime")
                        ))
...
//        $endDate = ($turnsEnd) ? $turnsEnd->end_datetime : $datenow . " 23:59:59";
    ...
    /**
     * Array de ids de turno en una fehc ao rango de fechas
     * @param type $microsite_id
     * @return array   IDS
     */
    public static function TurnIdsCalendar($microsite_id, $start_date = null, $end_date = null) {

        $realDate = ($start_date == null) ? CalendarHelper::realDate($microsite_id) : CalendarHelper::realDate($microsite_id, $start_date);
        $startDate = Carbon::parse($realDate);
        $endDate = ($end_date == null) ? $startDate->copy() : Carbon::parse($end_date);

        $turnIDs = [];
        
        if (strcmp($startDate->toDateString(), $endDate->toDateString()) == 1) {
            return [];
        }
        if (strcmp($startDate->toDateString(), $endDate->toDateString()) == 0) {

            $turnIDs = \App\res_turn_calendar::select('res_turn.id')
                    ->join("res_turn", "res_turn.id", "=", "res_turn_calendar.res_turn_id")
                    ->where(DB::raw("dayofweek(start_date)"), ($startDate->dayOfWeek + 1))
                    ->where("res_turn.ms_microsite_id", $microsite_id)
                    ->where("start_date", "<=", $startDate->toDateString())
                    ->where("end_date", ">=", $startDate->toDateString())
                    ->orderBy('start_date', 'desc')
                    ->pluck('res_turn.id');
            return $turnIDs->toArray();
        }
        ...
        $turnIDs = \App\res_turn_calendar::select('res_turn.id')
                ...
                ->where(function($query)use($startDate, $endDate) {
                    return $query->where("start_date", "<=", $startDate->toDateString())->where("end_date", "<=", $startDate->toDateString())
                            ->orwhere("start_date", "<=", $endDate->toDateString())->where("end_date", ">=", $endDate->toDateString());
                })
                ->orderBy('start_date', 'desc')
                ->pluck('res_turn.id');
        return $turnIDs->toArray();
